...
COMTRADE fault records generated by the simulator (`.hdr`/`.cfg`/`.dat`, or a `.zip` of the three) are written under `FileStore/LD/<ldName>/COMTRADE/` for every Logical Device that has a disturbance recorder LN (`RDRE`, `RADR`, `RBDR` or `RDRS`). Whether those files are retrievable over the MMS File Service (`GetFileDirectory` / `GetFile`) is decided **entirely by the `<FileHandling>` element in the `.icd`** — no code change needed:

```xml
<!-- MMS file service enabled — COMTRADE files retrievable via GetFile/GetFileDirectory -->
<FileHandling mms="true" />

<!-- FTP only — MMS file service is NOT enabled, even though FileHandling is present -->
<FileHandling ftp="true" />

<!-- Both transports enabled -->
<FileHandling mms="true" ftp="true" />

<!-- <FileHandling> element absent → no file transfer capability at all -->
```

To test COMTRADE retrieval over MMS:

1. Make sure the `.icd`/`.cfg` in use declares `<FileHandling mms="true" />` (or `mms="true" ftp="true"`). With `ftp="true"` only, the MMS file service stays disabled.
2. Start the application — the MMS server comes up on port `102` and `FileServiceBasePath` is set to the app's `FileStore/` folder.
